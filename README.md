...
以下所有示例均基于 `papers/` 目录中的 10 篇联邦学习相关论文，由 DeepSeek deepseek-chat 生成。
...
[CityScholar] 2026-06-18 19:02:03 - INFO - 加载配置: LLM=deepseek-chat, 环境=production
[CityScholar] 2026-06-18 19:02:03 - INFO - 扫描目录: papers
[CityScholar] 2026-06-18 19:02:03 - INFO - 找到 10 个文档
[CityScholar] 2026-06-18 19:02:03 - INFO - [1/10] 处理: 王腾等-2023-联邦学习中的隐私保护技术研究综述.pdf (25 个文本块)
[CityScholar] 2026-06-18 19:02:03 - INFO - [2/10] 处理: Arazzi等-2025-Privacy_Preserving...pdf (34 个文本块)
...
[CityScholar] 2026-06-18 19:02:04 - INFO - 构建 TF-IDF 向量索引 (sklearn TfidfVectorizer)
[CityScholar] 2026-06-18 19:02:04 - INFO - 向量索引保存到 storage/vectors.npy
[CityScholar] 2026-06-18 19:02:04 - INFO - 知识图谱已构建: 18 个节点, 45 条边
[CityScholar] 2026-06-18 19:02:04 - INFO - 知识库构建完成: 194 个文本块, 来自 10 篇文档
```

### 6.2 搜索论文

```bash
$ python main.py papers "差分隐私"

匹配 "差分隐私" 的论文（共 6 篇）：

  [1] 侯俊康 等 - 2026 - 异构数据的差分隐私个性化联邦学习方案
  [2] 孙剑明和赵梦鑫 - 2024 - 边缘计算下差分隐私的应用研究综述
  [3] 张嘉怡 等 - 2026 - 满足差分隐私的纵向联邦子空间聚类方法
  [4] 张兴贺和陈思 - 2026 - 差分隐私联邦学习：个性化策略综述
  [5] 张兴贺和赵航 - 2026 - 面向异构数据的差分隐私个性化联邦学习综述
  [6] 张永强 等 - 2026 - 隐私优先：差分隐私在联邦学习中的角色
```

```bash
$ python main.py papers "王腾"

匹配 "王腾" 的论文（共 1 篇）：

  [1] 王腾 等 - 2023 - 联邦学习中的隐私保护技术研究综述
```
...
> 基于五篇论文证据，联邦学习中差分隐私面临的主要挑战可概括为三大核心矛盾：
>
> **1. 隐私保护与模型效能的固有权衡**
>
> 差分隐私通过向梯度注入噪声提供隐私保证，但这会直接影响模型精度。隐私预算越小（ε 越小），噪声越大，模型性能越差。在联邦学习场景下，"隐私保护度和模型精确度是一对矛盾" [王腾等, 2023]。
>
> **2. 数据异质性（Non-IID）加剧的挑战**
>
> 客户端数据的非独立同分布特性会导致客户端漂移，差分隐私的噪声会进一步干扰不稳定的本地更新。张兴贺等（2026）指出，"当数据非独立同分布时，聚合模型性能严重退化"。
>
> **3. 隐私保护与安全鲁棒性之间的冲突**
>
> 差分隐私的噪声会掩盖恶意客户端的中毒攻击特征。侯俊康等（2026）揭示了"安全聚合通常与差分隐私不兼容"的结构性矛盾。
>
> **知识盲区**：现有证据未提供三种隐私保护技术（DP、HE、SMC）在具体任务中的性能横向对比数据。

### 6.4 分析单篇论文

```bash
$ python main.py analyze "Li 等 - 2020"
```

**输出摘要**（置信度 95%）：

> **核心创新**：系统性地梳理了联邦学习在工业工程领域的应用现状，识别出六个关键研究前沿（通信效率、隐私保护、异构性处理等）。与同期综述不同，该论文以"应用落地"为导向，通过文献计量和引文分析方法描绘FL发展脉络。
>
> **方法论评价**：采用文献综述与引文分析相结合的方法，是合理的。但核心假设（引文关系能有效反映知识流动）存在局限。
>
> **局限性**：发表于2020年，分析结果已无法完全反映最新进展。对具体技术方案的比较和批判性评价不足。
>
> **综述写作指导**：适合放入"FL应用"章节，可作为核心参考文献。
>
> **推理过程**：
...
| 维度 | Li 等 (2020) 综述 | Arazzi 等 (2025) CAGM | Bao 等 (2023) FEDCOLLAB |
|:---|:---|:---|:---|
| **研究定位** | 综述性、奠基性，梳理FL核心挑战与应用 | 方法创新，聚焦 Non-IID 下的隐私与鲁棒聚合 | 方法创新，聚焦负迁移下的协作结构优化 |
| **核心方法** | 文献综述与引文分析 | 类感知梯度掩码 (CAGM)，无需客户端元数据 | FEDCOLLAB 框架，基于分布距离+数据量聚类 |
| **证据强度** | 中等（文献计量） | 高（多基准数据集实验） | 高（多数据集+理论误差界支撑） |
| **综述建议** | 作为开篇，建立知识基础 | 作为聚合算法层面的先进方案 | 作为协作结构层面的先进方案 |

**综合洞察**：解决Non-IID问题需要多层次协同优化：先通过FEDCOLLAB优化协作结构（宏观），再用CAGM设计鲁棒聚合算法（微观）。

### 6.6 生成综述提纲

```bash
$ python main.py outline "联邦学习中的隐私保护方法"
```

**输出摘要**：

> **候选题目**：
> 1. 联邦学习中的隐私保护方法：挑战、技术与未来方向
> 2. 面向异构数据的联邦学习隐私保护：技术综述
> 3. 差分隐私、同态加密与安全多方计算：联邦学习隐私保护技术比较研究
>
> **提纲结构**：
>
> **第一章：引言** — FL背景、隐私泄露风险、综述范围
> **第二章：差分隐私方法** — 基本原理、FL中的应用（本地/中心/远程DP）、预算分配、与Non-IID的交互
> **第三章：加密方法** — 同态加密、安全多方计算（SMC）、混合加密方案
> **第四章：协同设计与系统优化** — 隐私与鲁棒性协同设计（CAGM）、协作结构优化（FEDCOLLAB）、能效与隐私的权衡
> **第五章：挑战与未来方向** — 量化权衡、大模型时代的FL隐私保护、开放问题
...
运行后会在 `outputs/` 目录生成完整的 Markdown 报告，包含以上所有板块（问答、3篇论文分析、4篇论文比较、综述提纲），以及完整的证据片段和推理过程追踪。报告文件名格式：`联邦学习中的隐私保护方法_YYYYMMDD_HHMMSS.md`。
